...
Qs = [[np.concatenate([np.ones((1, W.size()[0])), np.zeros((1, W.size()[0]))]),
       np.ones((1, W.size()[1]))] for W in Ws]
...
for num_iter in range(10000):
    ...
    if num_iter % Q_update_gap == 0:# let us update Q less frequently
        ...
        Qs = [psgd.update_precond_scan(q[0], q[1], dw.data.numpy(), dg.data.numpy()) for (q, dw, dg) in zip(Qs, delta, hess_delta)]
    
    # update Ws
    pre_grads = [psgd.precond_grad_scan(q[0], q[1], g.data.numpy()) for (q, g) in zip(Qs, grads)]
