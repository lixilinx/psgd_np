...
for num_iter in range(10000):
    ...
    if num_iter % Q_update_gap == 0:# let us update Q less frequently
        ...
        Qs = [psgd.update_precond_diag(q, dw.data.numpy(), dg.data.numpy()) for (q, dw, dg) in zip(Qs, delta, hess_delta)]
    
    # update Ws
    pre_grads = [psgd.precond_grad_diag(q, g.data.numpy()) for (q, g) in zip(Qs, grads)]
